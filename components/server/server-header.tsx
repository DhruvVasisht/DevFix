    ...
    <div >
      <DropdownMenu>
        <DropdownMenuTrigger asChild>
          <button className="w-full text-md font-semibold px-3 flex items-center h-12 border-neutral-200 dark:border-neutral-800 border-b-[0.5px] hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50 transition focus:outline-none cursor-pointer">
            {server.name}
        ...
        <DropdownMenuContent className="w-56 text-xs font-medium text-black dark:text-white space-y-[2px]">
          ...
          {isAdmin && (
           ...
           {!isAdmin && (
            <DropdownMenuItem onClick={()=> onOpen("LeaveServer", { server })} className="text-red-400 flex justify-between cursor-pointer ">
